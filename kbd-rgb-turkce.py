...
EFFECT_DISPLAY = [
    ("Static",                   "static"),
    ("Breathing (Nefes)",        "breathing"),
    ("Wave (Dalga)",             "wave"),
    ("Reactive (Tepkisel)",      "reactive"),
    ("Cycle (Renk Döngüsü)",     "cycle"),
    ("Ripple (Dalgalanma)",      "ripple"),
    ("Rain (Yağmur)",            "rain"),
    ("Snake (Yılan)",            "snake"),
    ("Starlight (Yıldız Işığı)", "starlight"),
    ("Radar",                    "radar"),
    ("Vortex (Girdap)",          "vortex"),
    ("Heartbeat (Kalp Atışı)",   "heartbeat"),
    ("Fireworks (Havai Fişek)",  "fireworks"),
    ("Sine Wave (Sinüs)",        "sine_wave"),
    ("Neon",                     "neon"),
    ("Screen Sync (Ekran)",      "screen_sync"),
    ("Music Sync (Müzik)",       "music_sync"),
    ("Mic Sync (Mikrofon)",      "mic_sync"),
    ("CPU Sıcaklık Takibi",      "cpu_temp"),
]
...
def print_header():
    print(CLEAR, end="")
    print(colored("╔══════════════════════════════════════════════════════════════╗", CYAN))
    print(colored("║        Monster / Clevo  ·  Klavye RGB Yöneticisi            ║", CYAN, BOLD))
...
def int_range_validator(lo, hi):
    def v(s):
        try:
            ...
            pass
        print(colored(f"  ! Lütfen {lo}–{hi} arasında bir sayı girin.", RED))
        return None
    return v
...
def color_input():
    print()
    print(colored("  Renk Seçimi", BOLD, YELLOW))
    ...
    print(f"  {colored(str(len(names)+1).rjust(2), GREEN)}. Özel RGB değeri gir")
    print()
    choice = get_input(f"Seçim (1–{len(names)+1})", int_range_validator(1, len(names)+1), 3)
    if choice <= len(names):
...
def speed_input(default=5):
    ...
    val = get_input("Hız (1=yavaş … 10=hızlı)", int_range_validator(1, 10), default)
    return SPEED_MAP[val]


def effects_menu(cfg):
    print_header()
    print(colored("  Efekt Seçimi", BOLD, YELLOW))
    print(colored("  " + "─" * 56, DIM))
    print()
    ...
    print()
    choice = get_input(f"Seçim (1–{len(EFFECT_DISPLAY)})", int_range_validator(1, len(EFFECT_DISPLAY)), 1)
    ...
    print(colored(f"\n  ✓  {label} efekti başlatıldı.", GREEN))
    input(colored("\n  [Enter] Ana menüye dön...", DIM))


def brightness_menu(cfg):
    print_header()
    print(colored("  Parlaklık Ayarı", BOLD, YELLOW))
    print(colored("  " + "─" * 56, DIM))
    print()
    cur = cfg.get("brightness", 100)
    bar_filled = int(cur / 5)
    bar = "█" * bar_filled + "░" * (20 - bar_filled)
    print(f"  Mevcut: {colored(str(cur) + '%', CYAN)}  [{colored(bar, BLUE)}]")
    print()
    val = get_input("Yeni parlaklık (0–100)", int_range_validator(0, 100), cur)
    cfg["brightness"] = val
    set_brightness(val)
    save_config(cfg)
    print(colored(f"\n  ✓  Parlaklık %{val} olarak ayarlandı.", GREEN))
    input(colored("\n  [Enter] Ana menüye dön...", DIM))
...
def profiles_menu(cfg):
    ...
    while True:
        print_header()
        print(colored("  Profil Yönetimi", BOLD, YELLOW))
        print(colored("  " + "─" * 56, DIM))
        print()
        items = [
        ...
        print()
        choice = get_input("Seçim (1–5)", int_range_validator(1, 5), 5)

        if choice == 1:
            name = get_input("Profil adı")
            if name:
                profiles[name] = {k: v for k, v in cfg.items()}
                save_profiles(profiles)
                print(colored(f"\n  ✓  '{name}' profili kaydedildi.", GREEN))
            input(colored("  [Enter] devam...", DIM))

        elif choice == 2:
            if not profiles:
                print(colored("\n  ! Kayıtlı profil bulunamadı.", YELLOW))
                input(colored("  [Enter] devam...", DIM))
                continue
            print()
            ...
            print()
            pidx = get_input(f"Profil (1–{len(pnames)})", int_range_validator(1, len(pnames)), 1)
            ...
            print(colored(f"\n  ✓  '{pnames[pidx-1]}' profili yüklendi.", GREEN))
            input(colored("  [Enter] devam...", DIM))
            ...
            for i, pn in enumerate(pnames, 1):
                print(f"  {colored(str(i), GREEN)}. {pn}")
            print()
            pidx = get_input(f"Silinecek profil (1–{len(pnames)})", int_range_validator(1, len(pnames)), 1)
            del profiles[pnames[pidx - 1]]
            save_profiles(profiles)
            print(colored(f"\n  ✓  '{pnames[pidx-1]}' silindi.", GREEN))
            input(colored("  [Enter] devam...", DIM))

        elif choice == 4:
            print()
            if not profiles:
                print(colored("  Kayıtlı profil yok.", YELLOW))
            else:
...
def systemd_menu(cfg):
    print_header()
    print(colored("  Systemd Servis Yönetimi", BOLD, YELLOW))
    print(colored("  " + "─" * 56, DIM))
    print()
    items = [
        "Servis dosyası oluştur (mevcut ayarlara göre)",
    ...
    if choice == 1:
        ...
        if r.returncode == 0:
            print(colored("\n  ✓  Servis etkinleştirildi ve başlatıldı.", GREEN))
        else:
            print(colored(f"\n  ✗  Hata: {r.stderr.strip()}", RED))
        ...
        if r.returncode == 0:
            print(colored("\n  ✓  Servis durduruldu ve devre dışı bırakıldı.", GREEN))
        else:
            print(colored(f"\n  ✗  Hata: {r.stderr.strip()}", RED))
...
def info_menu():
    print_header()
    print(colored("  Sistem Bilgisi & Yardım", BOLD, YELLOW))
    print(colored("  " + "─" * 56, DIM))
    print()
    ...
    if missing:
        ...
        print(colored("  ✓  brightnessctl ve cava kurulu.", GREEN))

    print()
    led_ok = Path(LED_PATH).exists()
    status_str = colored("✓ Erişilebilir", GREEN) if led_ok else colored("✗ Bulunamadı", RED)
    print(f"  LED Yolu : {colored(LED_PATH, DIM)}")
    print(f"  Durum    : {status_str}")

    temp = _read_cpu_temp()
    if temp is not None:
        print(f"  CPU Sıcaklık: {colored(f'{temp:.1f} °C', CYAN)}")

    print()
    print(colored("  ─── udev Kuralı (şifresiz erişim) ───────────────────────", DIM))
    rule = (
    ...
    print()
    print(colored(
        "  ─── Screen Sync için PIL gerekli ────────────────────────",
        DIM,
    ))
    print(colored("  sudo pacman -S python-pillow scrot", DIM))
    print()
    input(colored("  [Enter] Ana menüye dön...", DIM))
...
def main():
    ...
    while True:
        ...
        print(f"  Efekt      : {colored(eff, CYAN, BOLD)}  {swatch}")
        print(f"  Parlaklık  : {colored(str(br) + '%', CYAN)}  [{colored(bar, BLUE)}]")
        print(colored("  " + "─" * 56, DIM))
        print()
        ...
        menu_items = [
            ...
            "Profil Yönetimi",
            "Systemd Servis",
            "Sistem Bilgisi & Yardım",
            "Çıkış",
        ]
        print(colored("  Ana Menü", BOLD, YELLOW))
        print(colored("  " + "─" * 56, DIM))
        for i, item in enumerate(menu_items, 1):
            print(f"  {colored(str(i), GREEN)}. {item}")
        print()

        choice = get_input("Seçim (1–6)", int_range_validator(1, 6), 6)
        ...
        if choice == 1:
            ...
            stop_current_effect()
            print(colored("\n  İyi günler! Klavye son rengiyle bırakıldı. ✓\n", GREEN))
            sys.exit(0)
